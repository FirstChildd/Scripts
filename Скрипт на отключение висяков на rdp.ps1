...
foreach ($line in $lines) {
    ...
    if ($cleanLine -match '\b(\d+)\b') {
        ...
        $words = $beforeId -split '\s+', 3 # максимум 3, но нам нужно не больше 2
        $sessionName = ""
        $userName = ""
        ...
        if ($words.Count -eq 0) {
            ...
            # одно слово: определяем, имя сессии это или пользователь
            $word = $words[0]
            # Известные имена сессий (можно дополнить)
            ...
            # два или более слова: первое - имя сессии, второе - имя пользователя
            $sessionName = $words[0]
            $userName = $words[1]
        }
...
$confirmation = Read-Host "`nСбросить эти сессии? (y/n)"
